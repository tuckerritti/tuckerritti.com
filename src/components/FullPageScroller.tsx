...
export const useFullPageScroller = (): FullPageApiType => {
  // Use the context to get the api
  const context = React.useContext(FullPageScrollerContext);
  if (!context) {
    throw new Error('useFullPageScroller must be used within a FullPageScroller');
  }
  return context;
};

// Create a context to pass the API to child components
const FullPageScrollerContext = React.createContext<FullPageApiType | null>(null);
...
const FullPageScroller: React.FC<FullPageScrollerProps> = ({ 
  ...
  // Function to handle moving to a specific section
  ...
  // Function to move down one section
  const moveSectionDown = () => {
    moveTo(currentSection + 1);
  };
  
  // Function to move up one section
  const moveSectionUp = () => {
    moveTo(currentSection - 1);
  };
            ...
            {sections.map((_, index) => (
              <div
                ...
                style={{
                  ...
                  borderRadius: '50%',
                  cursor: 'pointer',
                  backgroundColor: currentSection === index ? '#FBBF24' : 'rgba(255, 255, 255, 0.5)'
                }}
